...
export async function MonthlyDashboard({ month }: { month: string }) {
  ...
  return <section className="dashboard" aria-label="Painel financeiro" id="dashboard">
    ...
    <div className="grid dashboard-grid">
      ...
      <article className="card dashboard-card"><h3>Quem pagou no mês</h3><div className="split-chart">{dashboard.personTotals.map((item, index) => <div key={item.person}><div className="split-chart-label"><span>{formatPerson(item.person)}</span><strong>{currency.format(item.value)}</strong></div><div className="progress"><span style={{ width: `${dashboard.monthTotal ? (item.value / dashboard.monthTotal) * 100 : 0}%`, background: index === 0 ? "#176b87" : "#e67e22" }} /></div></div>)}</div></article>
    ...
    <div className="grid insights-grid">
      <article className="card"><h3>Gastos por categoria</h3>{dashboard.categoryTotals.length === 0 ? <p className="note">Registre despesas para ver a distribuição.</p> : <div className="bar-list">{dashboard.categoryTotals.map(([name, value], index) => <div key={name}><div className="bar-label"><span><i style={{ background: categoryColors[index % categoryColors.length] }} />{name}</span><strong>{currency.format(value)}</strong></div><div className="progress"><span style={{ width: `${(value / maxCategoryTotal) * 100}%`, background: categoryColors[index % categoryColors.length] }} /></div></div>)}</div>}</article>
      <article className="card"><h3>Últimos seis meses</h3><div className="trend-chart">{dashboard.monthlyTrend.map((item) => <div key={item.date}><span className="trend-value">{item.value ? currency.format(item.value) : "–"}</span><div className="trend-column"><span style={{ height: `${(item.value / maxTrendValue) * 100}%` }} /></div><small>{monthFormatter.format(new Date(item.date))}</small></div>)}</div></article>
    </div>
  </section>;
}
