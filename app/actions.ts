...
import { deleteExpenseRecords } from "@/features/expenses/services/delete-expense";
...
export async function updateExpense(formData: FormData) {
  ...
  const updated = await updateExpenseRecord(id, input, scope);
  updateTag(dashboardCacheTag);
  revalidatePath("/");
...
export async function deleteExpense(formData: FormData) {
  ...
  const scope = parseExpenseMutationScope(formData.get("mutationScope"));
  await deleteExpenseRecords(id, scope);
  updateTag(dashboardCacheTag);
  revalidatePath("/");
